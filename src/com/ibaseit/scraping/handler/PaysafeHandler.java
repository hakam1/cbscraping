...
public class PaysafeHandler implements ResponseHandler {
	...
	public String handleResponse(HttpResponse response,
		...
		for (Element table : Jsoup.parse(
				page.substring(page.split("<table")[0].length())).getElementsByTag(
				"table")) {
	...
	private LogSheetData map(PaysafeHtmlData paysafe) {
		...
		try {
			logData.setInput_date(new java.sql.Date(new SimpleDateFormat(
					"MM/dd/yyyy", Locale.US).parse(paysafe.getInputDate()).getTime()));
			logData.setTransaction_date(new java.sql.Date(new SimpleDateFormat(
					"MM/dd/yyyy", Locale.US).parse(paysafe.getTransDate()).getTime()));

			logData.setCase_number(paysafe.getCaseNumber());
			logData.setCc_first_6(paysafe.getFirstSixAndLastFour().substring(0, 6));
			logData.setCc_last_4(paysafe.getFirstSixAndLastFour().substring(
					paysafe.getFirstSixAndLastFour().length() - 4,
					paysafe.getFirstSixAndLastFour().length()));
			logData.setAmount(Double.parseDouble(paysafe.getAmount().split("\\$")[1]
					.replace(")", "")));
